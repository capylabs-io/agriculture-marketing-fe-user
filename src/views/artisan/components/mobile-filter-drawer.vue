<template>
  <div class="full-height">
    <div>
      <div class="d-flex align-center justify-space-between">
        <div
          ...
          @click="artisanStore.mobileFilterDrawer = true"
        >
          <v-icon class="mr-2 mb-1">mdi-filter-variant</v-icon> Chọn Bộ lọc ({{
            artisanStore.allFilters.length
          }})
        </div>
        <v-btn
          ...
          elevation="0"
          text
          @click="artisanStore.resetFilter()"
          >Xóa Bộ lọc</v-btn
        >
      </div>
      <div
        class="mt-2 d-flex flex-wrap gap-8"
        v-if="artisanStore.allFilters.length > 0"
      >
        <v-chip
          color="primary"
          v-for="(filter, index) in artisanStore.allFilters"
          :key="index"
          small
        >
          {{ filter }}</v-chip
    ...
    <v-navigation-drawer
      ...
      v-model="artisanStore.mobileFilterDrawer"
      app
      fixed
    >
      <div class="px-6 py-3">
        <div class="d-flex justify-space-between align-center py-3">
          ...
          <div>
            ...
            <v-btn
              ...
              color="primary"
              small
              @click="artisanStore.mobileFilterDrawer = false"
              >Đóng</v-btn
            >
          </div>
        </div>
        ...
        <div class="py-2">
          <v-expansion-panels flat accordion>
            <v-expansion-panel>
              ...
              <v-expansion-panel-content>
                <v-checkbox
                  v-for="category in artisanStore.categories"
          ...
          <v-expansion-panels flat accordion class="mt-4">
            <v-expansion-panel>
              ...
              <v-expansion-panel-content>
                <v-checkbox
                  v-model="artisanStore.filterPrice"
                  value="lowerThan500k"
                  hide-details
                >
                  <template v-slot:label>
