    ...
    async fetchsearchCodes(code) {
      try {
        ...
        const mappedProducts = products
          .filter((product) => product.attributes.code.toLowerCase().includes(code.trim().toLowerCase()))
          .map((product) => {
            return {
              //   id: product.id,
        ...
        const mappedSupplies = supplies
          .filter((supply) => supply.attributes.code.toLowerCase().includes(code.trim().toLowerCase()))
          .map((supply) => {
            return {
              //   id: supply.id,
        ...
        const mappedSeeds = seeds
          .filter((seed) => seed.attributes.code.toLowerCase().includes(code.trim().toLowerCase()))
          .map((seed) => {
            return {
              //   id: seed.id,
